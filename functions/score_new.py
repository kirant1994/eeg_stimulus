...
def score(sample, truth, model, trans, nnet_dict):
    lang = truth['lang']
    nnet_feats = nnet_dict[lang]
    model = model[lang]
    block = truth['block']
    prompts = pd.read_excel('data/prompts.xlsx')
    block_ids = np.unique(prompts[prompts['Block'] == block]['Id'].values)
    print(block)
    print(block_ids)
    check_list = list()
    for block_id in block_ids:
        label = labels[labels['Id'] == block_id]['Label'].values[0]
        for nnet_feat in nnet_feats:
            if nnet_feat['label'] == label:
                check_list.append(nnet_feat)

    # The sample in the 0th position is the sample word. The following 12 samples are the checklist.
    arg_for_fbank = {'path':sample['path'], 'label':sample['label'], 'trans':trans[lang]}
    arg_after_fbank = makeFeature(arg_for_fbank)

    x = arg_after_fbank['feat']
    y = model.predict(x, batch_size=2048)
    arg_after_fbank['nnet'] = y

    args_for_dtw = list()
    s_label = arg_after_fbank['label']
    s_feat = arg_after_fbank['nnet']
    for nnet_feat in check_list:
        args_for_dtw.append({'s_label':s_label, 's_feat':s_feat, 'w_label':nnet_feat['label'], 'w_feat':nnet_feat['nnet']})
    
    result_after_dtw = run_parallel(getDtw, args_for_dtw, n_workers=20)
    result_after_dtw = sorted(result_after_dtw, key=getKey)
    
    min_dist = result_after_dtw[0]['dist']
    max_dist = result_after_dtw[-1]['dist']
    print(min_dist, max_dist)

    for i, result in enumerate(result_after_dtw):
        print('{0:s}\t{1:s}\t{2:.3f}'.format(result['s_label'], result['w_label'], result['dist']))
